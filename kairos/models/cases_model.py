...
def get_case_by_log(file_id,case_id):
    try:
        return list(db.cases.find({"file_id":file_id,"_id":ObjectId(case_id)}))
    except Exception as e:
        return e

def get_case(case_id):
    try:
        c = db.cases.find_one({"_id": ObjectId(case_id)})
        return c
...
def update_case(case_id,data):
    try:
        response = db.cases.cases.find_one_and_update({"_id": ObjectId(case_id)},{"$set": data})
        return response
    except Exception as e:
        return e
